-- Delivery Phase: show every real available courier to the merchant.
-- Zone/city matching is used for ranking only; it must not hide otherwise
-- eligible real couriers from the selection list.
...
BEGIN
    ...
    ORDER BY
        CASE
            WHEN d.zone_id = v_zone_id THEN 0
            WHEN v_zone_city IS NOT NULL AND driver_zone.city = v_zone_city THEN 1
            ELSE 2
        END,
        d.rating DESC NULLS LAST,
        d.delivered_count DESC NULLS LAST,
        COALESCE(NULLIF(TRIM(d.full_name), ''), NULLIF(TRIM(CONCAT_WS(' ', d.first_name, d.last_name)), ''), 'موصل') ASC;
END;
$$;
...
IS 'Returns all real, active, online, available couriers for a merchant order; ranks matching zones first and excludes demo and legacy QA accounts.';
