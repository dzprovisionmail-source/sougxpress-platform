-- Allow support staff to inspect a participant card in conversations they can see.
-- Regular users keep the original participant-only access check.
...
DECLARE
  ...
  v_other_id UUID;
  v_is_staff BOOLEAN := public.is_support_staff();
BEGIN
  ...
  IF v_is_staff THEN
    SELECT CASE
      WHEN p_profile_id = c.participant_one THEN c.participant_one
      WHEN p_profile_id = c.participant_two THEN c.participant_two
      ELSE NULL
    END
    INTO v_other_id
    FROM public.chat_conversations c
    WHERE c.id = p_conversation_id
      AND c.conversation_type IN ('commercial', 'support');
  ELSE
    SELECT CASE
  ...
  WITH profile_base AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE (st.merchant_id = p.id OR st.created_by = p.id)
        AND st.deleted_at IS NULL
      ORDER BY CASE WHEN st.merchant_id = p.id THEN 0 ELSE 1 END, st.created_at ASC
      LIMIT 1
  ...
  SELECT
    ...
    CASE
      WHEN pb.role = 'customer' THEN (
        ...
        WHERE o.customer_id = pb.id AND o.status NOT IN ('cancelled', 'rejected')
      )
      WHEN pb.role = 'merchant' THEN (
        SELECT COUNT(*)::INTEGER FROM public.orders o
        JOIN public.stores st ON st.id = o.store_id
        WHERE st.merchant_id = pb.id AND o.status NOT IN ('cancelled', 'rejected')
      )
      WHEN pb.role IN ('driver', 'courier') THEN (
        SELECT COUNT(*)::INTEGER FROM public.delivery_assignments da
        WHERE da.driver_id = pb.id AND da.status = 'delivered'
      )
      ELSE 0
    END
