...
-- Fix customer↔merchant chat eligibility for the actual schema:
-- stores.merchant_id references merchants.id, while chat participants use profiles.id.
-- Keep the existing order/favorite rules and all other pairings unchanged.
...
BEGIN
    ...
    IF p_relationship_type = 'customer_merchant' THEN
        ...
        -- The stores table uses merchants.id, not profiles.id. Resolve the
        -- merchant role record through the profile email without exposing it.
        SELECT m.id
          INTO v_merchant_id
          FROM public.merchants m
          JOIN public.profiles p
            ON lower(trim(p.email)) = lower(trim(m.email))
         WHERE p.id = v_merchant_profile_id
         LIMIT 1;
            ...
            RETURN EXISTS (
                SELECT 1
                 ...
                 WHERE o.id = p_reference_id
                   ...
                   AND (s.merchant_id = v_merchant_id OR s.merchant_id = v_merchant_profile_id)
            );
        END IF;
        ...
        OR EXISTS (
            SELECT 1
             ...
             WHERE mf.merchant_id = v_merchant_profile_id
    ...
    IF p_relationship_type = 'customer_courier' THEN
        ...
        OR EXISTS (
            SELECT 1
              FROM public.delivery_assignments da
              JOIN public.orders o ON o.id = da.order_id
             WHERE o.customer_id = v_customer_id AND da.driver_id = v_driver_id
        );
    END IF;
    ...
    IF p_relationship_type = 'merchant_courier' THEN
        ...
        IF p_reference_id IS NOT NULL AND EXISTS (
            SELECT 1
              ...
              JOIN public.stores s ON s.id = o.store_id
             WHERE (o.id = p_reference_id OR da.order_id = p_reference_id)
               AND (s.merchant_id = v_merchant_profile_id)
               AND da.driver_id = v_driver_id
        ) THEN
            RETURN true;
        END IF;
        ...
        OR EXISTS (
            SELECT 1
              ...
              JOIN public.orders o ON o.id = da.order_id
              JOIN public.stores s ON s.id = o.store_id
             WHERE s.merchant_id = v_merchant_profile_id
               AND da.driver_id = v_driver_id
        );
    END IF;
...
-- Verification intent (read-only after applying): customer↔merchant must be
-- true when either side has the qualifying favorite or a related order.
-- No customer↔merchant phone permission is introduced by this migration.
