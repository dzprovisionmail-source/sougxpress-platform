...
import { getAdminCustomers, updateAdminCustomerStatus } from "@/services/admin.service";
...
export default function AdminCustomersScreen() {
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    setActionError(null);
    const { error: err } = await updateAdminCustomerStatus(id, newStatus);
  ...
  const currentStatus = (selected?.["status"] as CustomerStatus) ?? "active";
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              const gold = item["is_gold_member"] === true;
              return (
                <AdminListItem
                  title={String(item["full_name"] ?? "") + (gold ? " ⭐" : "")}
          ...
          <View style={[styles.sheet, { backgroundColor: colors.bgSurface }]}>
            ...
            {actionLoading ? (
              ...
              <View style={{ gap: 10 }}>
                ...
                {currentStatus === "active" && (
                  <ActionButton label="إيقاف مؤقت" color={colors.warning} onPress={() => handleStatusChange(String(selected?.["id"]), "suspended")} tokens={tokens} />
                )}
                {currentStatus !== "banned" && (
                  <ActionButton label="حظر" color={colors.error} onPress={() => handleStatusChange(String(selected?.["id"]), "banned")} tokens={tokens} />
                )}
